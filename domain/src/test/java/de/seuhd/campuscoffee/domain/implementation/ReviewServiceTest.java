...
public class ReviewServiceTest {
    ...
    void userCannotCreateMoreThanOneReviewPerPos() {
        // given
        Review existingReview = TestFixtures.getReviewFixtures().getFirst();
        Pos pos = existingReview.pos();
        User author = existingReview.author();
        assertNotNull(pos.getId());

        Review newReview = existingReview.toBuilder().id(null).build();
        when(posDataService.getById(pos.getId())).thenReturn(pos);
        when(reviewDataService.filter(pos, author)).thenReturn(List.of(existingReview));

        // when, then
        assertThrows(ValidationException.class, () -> reviewService.upsert(newReview)
        );
        verify(posDataService).getById(pos.getId());
